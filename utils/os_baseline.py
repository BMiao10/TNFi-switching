...
class NotesDataset(Dataset):               
    def __init__(self, data_fpath):  
        ...
        # TESTING: Remove previously run values
        '''rm_fpath = "./data/ncdmard/baseline/2024-03-29_starling-7b-beta_reasons-provided_test_1.csv"
        print("Removing notes from", rm_fpath)
        rm_df = pd.read_csv(rm_fpath, index_col=0)
        pts_df = pts_df[~pts_df["note_deid_note_key"].isin(list(rm_df.index))]
        '''
        print("Remaining notes", pts_df.shape)
